...
def load_data():
    try:
        ...
        res = supabase.table("v_ton").select("*").execute()
...
if not df_hien_thi.empty:
    ...
    with col1:
        tu_khoa = st.text_input("🔍 Tìm mã / tên hàng", placeholder="VD: 028739 hoặc ghế...")
    with col2:
        st.write("")
        st.success(f"🚀 Đã tải thành công {len(df_hien_thi):,} sản phẩm từ cơ sở dữ liệu.")
